...
# 除外なし: MAX_POSE_POINTCLOUD_DT_SEC= ./compare_mean_pose.sh ... (空で --no-pose-pointcloud-dt-filter)
#
# Usage:
#   ./compare_mean_pose.sh <ndt_mean_pose.yaml> <mean_pose.json> [--yaml-out path] [--json-out path]

set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
: "${MAX_POSE_POINTCLOUD_DT_SEC:=0.21}"

CMD=(python3 "$SCRIPT_DIR/compare_mean_pose.py")
